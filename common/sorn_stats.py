...
def characteristic_path_length(graph_matrix):
    N = shape(graph_matrix)[0]
    #BUGFIX--> use transpose to get correct direction of weights
...
class ActivityInhibStat(AbstractStat):
    ...
    def __init__(self):
        self.name = 'activityInh'
        self.collection = 'gather'
    def clear(self,c,sorn):
        c.activityInh = zeros(sorn.c.N_steps)
        self.step = 0
    def add(self,c,sorn):
...
class CountingLetterStat(AbstractStat):
    ...
    def __init__(self):
        ...
        self.collection = 'gather'
    def clear(self,c,sorn):
        c.inputletter = zeros(sorn.c.C_steps)
        self.step = 0
    def add(self,c,sorn):
...
class BayesStat(AbstractStat):
    ...
    def clear(self,c,sorn):
        ...
        #self.M_neurons = where(sorn.W_eu.W[:,sorn.source.source.lookup['M']]==1)[0]
        #self.N_neurons = where(sorn.W_eu.W[:,sorn.source.source.lookup['N']]==1)[0]
    def report(self,c,sorn):
        ...
        Xtrain = c.spikes[:,steps_plastic:N_inputtrain_steps].T
        Xtest = c.spikes[:,N_inputtrain_steps:].T
        assert(shape(Xtest)[0] == N_test_steps)
        inputi_train = c.inputindex[steps_plastic:N_inputtrain_steps]
        ...
        # For each index, take the same number of states from the
        # end phase of learning to avoid a bias in comparing states
        norm_Xtrain = np.zeros((min_letter_count*(maxindex+1),shape(Xtrain)[1]))
        ...
        #raw_predictions[1:,1] = sum((sorn.W_ee*Xtest[:-1].T)[self.N_neurons],0)
        ...
        for (s_word,s_index) in zip(source.words,source.glob_ind):
            ...
            output_drive[i,0] += mean(raw_predictions[indices,0]) # M predicted
            output_drive[i,1] += mean(raw_predictions[indices,1]) # N predicted
            decisions[i,0]    += mean(raw_predictions[indices,0]>raw_predictions[indices,1])
            decisions[i,1]    += mean(raw_predictions[indices,1]>=raw_predictions[indices,0])
            output_std[i,0]   +=  std(raw_predictions[indices,0])
            output_std[i,1]   +=  std(raw_predictions[indices,1])
            denom[i]          += 1
        # Becasue some words occur more than once...
...
class AttractorDynamicsStat(AbstractStat):
    ...
    def report(self,c,sorn):
        ...
        min_trials = inf
        for i in range(int(max(c.inputi_test))):
        ...
        for (p,pp) in enumerate(arange(0,word_length)):
            ...
            denom = np.zeros((N_fracs))
            for (w,(s_word,s_index)) in enumerate(zip(source.words,source.glob_ind)):
                i = ''.join(letters_for_frac).find(s_word[0])
                indices = find(inputi_test==s_index+pred_pos)
...
class TrialBayesStat(AbstractStat):
    ...
    def report(self,c,sorn):
        ...
        start_indices = start_indices[(start_indices>STA_window) * # * is element-wise AND
...
class SpontBayesStat(AbstractStat):
    ...
    def report(self,c,sorn):
        ...
        for (s_word,s_index) in zip(source.words,source.glob_ind):
            ...
            pred_indices = pred_indices[pred_indices<shape(inputi_test)[0]]
    ...
    def report(self,c,sorn):
        ...
        for (w,word) in enumerate(source.words):
            ...
            for t in range(pred_window):
                ...
                try:
                    # old
                    #~ classifier = linalg.pinv(sp_train).dot(Xt_train)
                ...
                # TODO change baseline to shuffled pred as for trialbayes
                # Baseline = STA
                ...
                def match(x,y): # instead of pearsonr - lower bound = 1-h.ip, spont pred always better
                    assert(shape(x) == shape(y))
                    x = x>0
                    y = y>0
                    return sum(x==y)/(1.0*shape(x)[0])
                if not sorn.c.stats.match:
                    correlations[w,t,0] = stats.pearsonr(Xt_pred.flatten(),Xt_test.flatten())[0]
                    correlations[w,t,1] = stats.pearsonr(base_pred.flatten(),Xt_test.flatten())[0]
                else:
    ...
    def add(self,c,sorn):
        ...
        if sorn.c.stats.has_key('only_last_spikes'):
            new_step = self.step - (sorn.c.N_steps\
                                    -sorn.c.stats.only_last_spikes)
            if new_step >= 0:
                c[self.sattr][:,new_step] = spikes_ihn
        else:
            c[self.sattr][:,self.step] = spikes_ihn
        self.step += 1
    def report(self,c,sorn):
        if sorn.c.stats.save_spikes:
            return c[self.sattr]
...
class ISIsStat(AbstractStat):
    ...
    def add(self,c,sorn):
        if ((self.step > self.interval[0] and
            ...
            #~ if(sorn.x[self.neuron] == 0):
                #~ self.isi += 1
            #~ else:
                #~ if(self.isi < 50):
                    #~ self.ISIs[self.isi] += 1
                #~ self.isi = 0
    ...
    def report(self,c,sorn):
        ...
        for s in xrange(rec_steps):
            for i in xrange(sorn.c.N_e):
                ...
                #~ overlaps = c.norm_last_input_spikes.T.dot(c.SVD_U[s,:,i])
    ...
    def report(self,c,sorn):
        ...
        for s in xrange(rec_steps):
            for i in xrange(sorn.c.N_e):
                ...
                #~ similar_input[s,i] = argmax(index_overlap)
                # No big difference to this, but probably more robust
                max_overlap = argmax(c.norm_last_input_spikes.T.dot(c.SVD_V[s,:,i])) # euclidean norm w/0 sqrt
                similar_input[s,i] = c.norm_last_input_index[max_overlap]
        '''
    ...
    def add(self,c,sorn):
        for (i,(start,stop)) in enumerate(self.intervals):
            if self.step > start and self.step < stop:
                ...
                self.patterns[i,packbits(sorn.x[self.subset])] += 1
        self.step += 1
    def report(self,c,sorn):
...
class WeightHistoryStat(HistoryStat):
    def add(self,c,obj):
        if not (c.history[self.counter] % self.record_every_nth):
            c.history[self.name].append(np.copy(_getvar(obj,self.var).get_synapses()))
