#!/usr/bin/env python3
"""Run ``common/test_single.py`` repeatedly while sweeping SORN parameters.

This driver is written for **python 3** and only uses the standard library, but
the SORN model code itself is python 2.7 (print statements, ``cPickle``, ...),
so every simulation is launched as a subprocess with a python 2 interpreter
(auto-detected, or given with ``--python``).

For every point of the sweep a small parameter module is generated in
``sweep_params/``.  It imports the base parameter file and overrides only the
swept values, e.g.::

    from delpapa.param_FrozenPlasticity import *
    c.h_ip = 0.02
    c.W_ei.h_ip = 0.02

so nothing in the existing code has to be modified.  The generated module is
passed to ``test_single.py`` exactly like a hand written parameter file, and is
archived together with the results by the usual backup machinery.

Examples
--------
Sweep ``h_ip`` from 0.02 to 0.2 in steps of 0.02 (10 simulations) with the
frozen plasticity experiment -- this is also what the defaults do::

    python3 run_sweep.py

Ten repetitions per value, four simulations at a time::

    python3 run_sweep.py --repeats 10 --jobs 4

Short test runs of a different experiment, several parameters at once::

    python3 run_sweep.py --param delpapa.param_Zheng2013 \
        --sweep h_ip=0.02:0.2:0.02 --sweep N_e=100,200,400 \
        --set N_steps=100000 --set steps_plastic=50000

Check what would be executed without running anything::
...
By default the backup directory produced by each simulation is moved into::
    ...
    <sweep-dir>/<label>/<repetition>/common/result.h5

(repetitions are numbered from 1), which is the layout the avalanche scripts in
``delpapa/avalanches`` expect -- point their ``exper_path`` at
``<sweep-dir>/<label>/`` and set ``number_of_files`` to the number of
repetitions.  Use ``--layout raw`` to leave the backup directories untouched.
A ``manifest.json`` in the sweep directory records the parameters, status,
runtime and output directory of every run, and ``--resume`` re-uses it to skip
